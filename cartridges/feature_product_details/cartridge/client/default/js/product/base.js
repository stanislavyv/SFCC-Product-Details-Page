...
function getQuantitySelector($el) {
    ...
    if ($el && $(".set-items").length) {
        ...
        quantitySelected = $(".quantity-select .selected");
    }
...
function getQuantitySelected($el) {
    return getQuantitySelector($el).attr('value');
}
...
function updateQuantities(quantities, $productContainer) {
    if ($productContainer.parent(".bonus-product-item").length <= 0) {
        ...
        const selected = quantities.find((q, i) => {
            if (q.selected) {
                if (i > 0) {
                    prev = quantities[i - 1];
                }
        ...
        const $qtyElement = $productContainer.find(
            ".quantity-select .selected"
        );
        $qtyElement.attr("value", selected.value);
        $qtyElement.text(selected.value);
        $qtyElement.data("url", selected.url);

        const $minusButton = $qtyElement.prev();
        if (prev) {
            $minusButton.prop("disabled", false);

            $minusButton.val(prev.value);
            $minusButton.data("url", prev.url);
        } else {
            $minusButton.prop("disabled", true);
        }

        const $plusButton = $qtyElement.next();
        if (next) {
            $plusButton.prop("disabled", false);

            $plusButton.val(next.value);
            $plusButton.data("url", next.url);
        } else {
    ...
    selectQuantity: function () {
        ...
        $quantityButtons.each(function (i, button) {
            ...
            $button.on("click", function () {
                attributeSelect($button.data("url"), $productContainer);
            });
        });
    },
    ...
    availability: function () {
        $(document).on("click", ".quantity-btn", function (e) {
            ...
            if ($(".bundle-items", $productContainer).length === 0) {
                attributeSelect(
                    $(this).closest(".selected").data("url"),
                    $productContainer
                );
            }
        });
    ...
    selectBonusProduct: function () {
        $(document).on("click", ".select-bonus-product", function () {
            ...
            var submittedQty = parseInt(
                $choiceOfBonusProduct
                    .find(".bonus-quantity-select .selected")
                    .val(),
                10
            );
            var totalQty = 0;
